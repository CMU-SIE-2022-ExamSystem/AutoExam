...
func getExam(collection *mongo.Collection, exam *AutoGenerated, id1, id2 string) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func genStudent(collection *mongo.Collection, exam *AutoGenerated, student *Student, id1, id2, id3 string) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
