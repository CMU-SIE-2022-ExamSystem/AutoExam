...
const Question = ({questionData} : {questionData: any}) => {
    const subQuestions = questionData.questions.map((subQuestionData: any) => {
        if (subQuestionData.questionType === "single-choice") return (<SingleChoice data={subQuestionData} />);
        if (subQuestionData.questionType === "multiple-choice") return (<MultipleChoice data={subQuestionData} />);
        if (subQuestionData.questionType === "single-blank") return (<SingleBlank data={subQuestionData} />);
        if (subQuestionData.questionType === "multiple-blank") return (<MultipleBlank data={subQuestionData} />);
    });
